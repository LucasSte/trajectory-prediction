...
def train_models(look_back, look_forth, output_dims, robot_model_name, ball_model_name):
    ...
    seq_predictor.fit(robot_x, y, epochs=2, batch_size=2048, callbacks=[batch_logs], validation_split=0.1)
    ...
    seq_predictor.compile(optimizer=tf.optimizers.Adam(learning_rate=lr_schedule), loss=SequenceLoss(), run_eagerly=False)
    batch_logs = BatchLogs()
    seq_predictor.fit([robot_x, ball_x, ball_mask], y, epochs=2, batch_size=2048, callbacks=[batch_logs], validation_split=0.1)
    seq_predictor.save_model(ball_model_name)
    plots(batch_logs)


train_models(15, 30, 2, 'robot_30_15', 'ball_30_15')
train_models(30, 60, 2, 'robot_30_60', 'ball_30_60')
